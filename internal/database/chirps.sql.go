...
const getAllChirpsOrderedByCreationDate = `-- name: GetAllChirpsOrderedByCreationDate :many
SELECT id, created_at, updated_at, body, user_id FROM chirps ORDER BY created_at
`

func (q *Queries) GetAllChirpsOrderedByCreationDate(ctx context.Context) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChirpsOrderedByCreationDate)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
